...
void main() {
  ...
  setUp(() {
    ...
    when(mockOrderManagementService.getAllOrdersForAdmin())
        .thenAnswer((_) async => [
          TestHelpers.createTestBasicOrderJson(id: 1, userId: 'user1'),
          TestHelpers.createTestBasicOrderJson(id: 2, userId: 'user2', status: 'delivered'),
        ]);

    when(mockOrderManagementService.getOrderStatistics())
        .thenAnswer((_) async => {
          'total_orders': 2,
    ...
    when(mockOrderManagementService.getValidStatuses())
        .thenReturn(['pending', 'confirmed', 'processing', 'shipped', 'delivered', 'cancelled', 'refunded']);
  });
  ...
  group('OrderStatusChip Widget Tests', () {
    testWidgets('should display correct status and color for pending order', (tester) async {
      await tester.pumpWidget(createTestWidget(
        const OrderStatusChip(status: 'pending'),
      ));
    ...
    testWidgets('should display correct status for delivered order', (tester) async {
      await tester.pumpWidget(createTestWidget(
        const OrderStatusChip(status: 'delivered'),
      ));

      expect(find.text('Zugestellt'), findsOneWidget);
    });

    testWidgets('should display unknown status for invalid status', (tester) async {
      await tester.pumpWidget(createTestWidget(
        const OrderStatusChip(status: 'invalid_status'),
      ));
  ...
  group('OrderFilterWidget Tests', () {
    testWidgets('should render filter components', (tester) async {
      await tester.pumpWidget(createTestWidget(
        const Scaffold(body: OrderFilterWidget()),
      ));
  ...
  group('OrderStatisticsWidget Tests', () {
    testWidgets('should display loading indicator when loading', (tester) async {
      ...
      await tester.pumpWidget(createTestWidget(
        const Scaffold(body: OrderStatisticsWidget()),
      ));
    ...
    testWidgets('should display empty message when no statistics', (tester) async {
      await tester.pumpWidget(createTestWidget(
        const Scaffold(body: OrderStatisticsWidget()),
      ));
    ...
    testWidgets('should display time period selector', (tester) async {
      ...
      expect(find.text('Heute'), findsNWidgets(2)); // One in statistics card, one in selector
  ...
  group('OrderListWidget Tests', () {
    testWidgets('should display loading indicator when loading', (tester) async {
      ...
      await tester.pumpWidget(createTestWidget(
        const Scaffold(body: OrderListWidget()),
      ));
  ...
  group('OrderManagementPage Integration Tests', () {
    ...
    testWidgets('should handle refresh button tap', (tester) async {
      ...
      verify(mockOrderManagementService.getAllOrdersForAdmin()).called(greaterThan(1));
    });
  });
}