...
void main() {
  group('Offline to Online Integration Tests', () {
    ...
    group('Hike Repository Offline-to-Online Flow', () {
      testWidgets('should transition from offline to online gracefully', (tester) async {
      ...
      testWidgets('should handle network failures gracefully during transition', (tester) async {
      ...
      testWidgets('should sync user-specific data correctly', (tester) async {
        ...
        final cachedUserHikes = [TestHelpers.createTestHike(id: 1, name: 'User Hike 1')];
        await offlineService.cacheHikeList(cachedUserHikes, listKey: 'user_$userId');
    ...
    group('Waypoint Repository Offline-to-Online Flow', () {
      testWidgets('should handle waypoint modifications during offline-to-online transition', (tester) async {
        // Phase 1: Cache waypoints offline
        const hikeId = 1;
        final cachedWaypoints = TestHelpers.createTestWaypoints(hikeId: hikeId);
        ...
        final newWaypoint = TestHelpers.createTestWaypoint(id: 999, name: 'Offline Added');
        ...
        when(mockConnectivityService.currentStatus)
            .thenReturn(NetworkStatus.connected_wifi);
        when(mockBackendApi.addWaypoint(any, any, orderIndex: anyNamed('orderIndex')))
            .thenAnswer((_) async {});
        when(mockBackendApi.getWaypointsForHike(hikeId))
      ...
      testWidgets('should handle waypoint deletions during offline-to-online transition', (tester) async {
        // Phase 1: Setup waypoints
        const hikeId = 1;
        final initialWaypoints = TestHelpers.createTestWaypoints(hikeId: hikeId);
        ...
        final localWaypoints = await offlineService.getCachedWaypoints(hikeId);
        expect(localWaypoints!.any((w) => w.id == waypointToDelete.id), isFalse);

        // Phase 3: Go online and sync
        when(mockConnectivityService.currentStatus)
            .thenReturn(NetworkStatus.connected_wifi);
        when(mockBackendApi.deleteWaypoint(waypointToDelete.id, hikeId))
            .thenAnswer((_) async {});
        ...
        expect(onlineWaypoints.any((w) => w.id == waypointToDelete.id), isFalse);
    ...
    group('Complex Integration Scenarios', () {
      testWidgets('should handle complete offline-to-online workflow', (tester) async {
        ...
        final offlineWaypoints = await waypointRepository.getWaypointsForHike(1);
        ...
        // Phase 2: Make offline modifications
        final newWaypoint = TestHelpers.createTestWaypoint(id: 999, name: 'Offline Added');
        await waypointRepository.addWaypoint(newWaypoint, 1, syncWhenOnline: true);
        ...
        final freshHikes = [...cachedHikes, TestHelpers.createTestHike(id: 999, name: 'Fresh Hike')];
        final freshWaypoints = [...cachedWaypoints, newWaypoint];

        when(mockBackendApi.fetchHikes())
            .thenAnswer((_) async => freshHikes);
        when(mockBackendApi.getWaypointsForHike(1))
        ...
        final onlineHikes = await hikeRepository.getAllAvailableHikes(forceRefresh: true);
        final onlineWaypoints = await waypointRepository.getWaypointsForHike(1, forceRefresh: true);
      ...
      testWidgets('should maintain data consistency during network interruptions', (tester) async {
        // Start online
        when(mockConnectivityService.currentStatus)
            .thenReturn(NetworkStatus.connected_wifi);
        ...
        final updatedHikes = [...initialHikes, TestHelpers.createTestHike(id: 999, name: 'Updated')];
        when(mockBackendApi.fetchHikes())
