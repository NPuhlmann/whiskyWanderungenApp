...
void main() {
  group('Commission Workflow Integration Tests', () {
    ...
    group('Complete Order-to-Commission Flow', () {
      test(
        'should complete entire workflow from shipped to delivered with commission',
        () async {
          ...
          when(
            mockCommissionService.getCommissionByOrderId('ORD-001'),
          ).thenAnswer((_) async => expectedCommission);
          ...
          final retrievedCommission = await mockCommissionService
              .getCommissionByOrderId('ORD-001');
    ...
    group('Commission Analytics Integration', () {
      test(
        'should generate analytics data after commission creation',
        () async {
    ...
    group('Error Handling and Recovery', () {
      ...
      test(
        'should handle missing commission data in order management',
        () async {
          // Arrange - Commission lookup returns null
          when(
            mockCommissionService.getCommissionByOrderId('ORD-MISSING'),
          ).thenAnswer((_) async => null);

          // Act - Should not throw an error
          final commission = await mockCommissionService.getCommissionByOrderId(
            'ORD-MISSING',
          );
    ...
    group('Business Rules Validation', () {
      ...
      test('should only create commissions for delivered orders', () async {
        ...
        when(
          ...
          (_) async => testOrder.copyWith(status: EnhancedOrderStatus.shipped),
        );
    ...
    group('Performance and Scale', () {
      test(
        'should handle multiple commission operations efficiently',
        () async {
          // Arrange - Simulate processing multiple orders
          final orders = List.generate(
            5,
            (index) => _createTestOrder(
              id: index + 1,
          ...
          final commissions = List.generate(
            5,
            (index) => TestHelpers.createTestCommission(
              id: index + 1,
              hikeId: 100 + index,
              companyId: 'test-company',
          ...
          for (int i = 0; i < orders.length; i++) {
            when(
              mockBackendApi.getEnhancedOrderById(i + 1),
          ...
          final results = await Future.wait(
            List.generate(
              5,
              (index) => workflow.transitionOrderStatus(
                orderId: index + 1,
                toStatus: EnhancedOrderStatus.delivered,
              ),
            ),
