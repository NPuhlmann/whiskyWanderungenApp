  ...
  group('MultiPaymentService - Basic Tests', () {
    ...
    group('Display Names and Icons', () {
      test('should return correct German display names', () {
        // Act & Assert
        expect(
          service.getPaymentMethodDisplayName(PaymentMethodType.card),
          'Kreditkarte',
        );
        expect(
          service.getPaymentMethodDisplayName(PaymentMethodType.applePay),
          'Apple Pay',
        );
        expect(
          service.getPaymentMethodDisplayName(PaymentMethodType.googlePay),
          'Google Pay',
        );
        // PayPal not in enum yet - future implementation
        expect(
          service.getPaymentMethodDisplayName(PaymentMethodType.sepaDebit),
          'SEPA Lastschrift',
        );
        expect(
          service.getPaymentMethodDisplayName(PaymentMethodType.sofort),
          'Sofort',
        );
        expect(
          service.getPaymentMethodDisplayName(PaymentMethodType.giropay),
          'Giropay',
        );
        expect(
          service.getPaymentMethodDisplayName(PaymentMethodType.ideal),
          'iDEAL',
        );
      });
      ...
      test('should return appropriate icons for payment methods', () {
        // Act & Assert
        expect(
          service.getPaymentMethodIcon(PaymentMethodType.card),
          'credit_card',
        );
        expect(
          service.getPaymentMethodIcon(PaymentMethodType.applePay),
          'apple',
        );
        expect(
          service.getPaymentMethodIcon(PaymentMethodType.googlePay),
          'google',
        );
        // PayPal icon test - future implementation
        expect(
          service.getPaymentMethodIcon(PaymentMethodType.sepaDebit),
          'account_balance',
        );
        expect(
          service.getPaymentMethodIcon(PaymentMethodType.sofort),
          'flash_on',
        );
        expect(
          service.getPaymentMethodIcon(PaymentMethodType.giropay),
          'euro_symbol',
        );
        expect(
          service.getPaymentMethodIcon(PaymentMethodType.ideal),
          'euro_symbol',
        );
      });
    });
    ...
    group('Payment Method Availability', () {
      test('should always return true for basic payment methods', () async {
        ...
        // Act & Assert
        expect(
          await service.isPaymentMethodAvailable(PaymentMethodType.card),
          isTrue,
        );
        expect(
          await service.isPaymentMethodAvailable(PaymentMethodType.applePay),
          isTrue,
        );
      });
    ...
    group('Basic Service Functionality', () {
      ...
      test('should handle invalid payment method gracefully', () async {
        ...
        expect(
          await service.isPaymentMethodAvailable(PaymentMethodType.sofort),
          isFalse,
        );
      });
    });
