...
void main() {
  group('WaypointRepository Tests', () {
    ...
    group('getWaypointsForHike', () {
      ...
      test(
        'should return list of waypoints when backend service succeeds',
        () async {
          // Arrange
          ...
          when(
            mockBackendApiService.getWaypointsForHike(testHikeId),
          ).thenAnswer((_) async => expectedWaypoints);

          // Act
          final result = await waypointRepository.getWaypointsForHike(
            testHikeId,
          );
      ...
      test(
        'should throw custom exception when backend service fails',
        () async {
          // Arrange
          ...
          try {
            ...
            expect(
              e.toString(),
              contains(
                'Fehler beim Abrufen der Wegpunkte für Wanderung $testHikeId',
              ),
            );
      ...
      test('should handle different hike IDs correctly', () async {
        ...
        when(
          mockBackendApiService.getWaypointsForHike(hikeId1),
        ).thenAnswer((_) async => waypoints1);
        when(
          mockBackendApiService.getWaypointsForHike(hikeId2),
    ...
    group('addWaypoint', () {
      ...
      test('should add waypoint successfully', () async {
        // Arrange
        when(
          mockBackendApiService.addWaypoint(testWaypoint, testHikeId),
        ).thenAnswer((_) async => {});
      ...
      test(
        ...
        () async {
          ...
          try {
            ...
            expect(
              e.toString(),
              contains('Fehler beim Hinzufügen des Wegpunkts'),
            );
            expect(e.toString(), contains(originalError));
          }
      ...
      test('should handle waypoint with images', () async {
        ...
        when(
          mockBackendApiService.addWaypoint(waypointWithImages, testHikeId),
        ).thenAnswer((_) async => {});
      ...
      test('should add waypoint with custom order index', () async {
        ...
        when(
          mockBackendApiService.addWaypoint(
            testWaypoint,
            testHikeId,
            orderIndex: customOrderIndex,
          ),
        ).thenAnswer((_) async => {});
      ...
      test(
        'should add waypoint without order index when not specified',
        () async {
          // Arrange
          when(
            mockBackendApiService.addWaypoint(
    ...
    group('updateWaypoint', () {
      ...
      test('should update waypoint successfully', () async {
        // Arrange
        when(
          mockBackendApiService.updateWaypoint(testWaypoint),
        ).thenAnswer((_) async => {});
      ...
      test(
        ...
        () async {
          ...
          try {
            ...
            expect(
              e.toString(),
              contains('Fehler beim Aktualisieren des Wegpunkts'),
            );
            expect(e.toString(), contains(originalError));
          }
      ...
      test('should handle updating visited status', () async {
        ...
        when(
          mockBackendApiService.updateWaypoint(visitedWaypoint),
        ).thenAnswer((_) async => {});
    ...
    group('deleteWaypoint', () {
      ...
      test('should delete waypoint successfully', () async {
        // Arrange
        when(
          mockBackendApiService.deleteWaypoint(testWaypointId, testHikeId),
        ).thenAnswer((_) async => {});
      ...
      test('should handle deleting non-existent waypoint', () async {
        ...
        when(
          mockBackendApiService.deleteWaypoint(nonExistentId, testHikeId),
      ...
      test(
        'should handle different waypoint and hike ID combinations',
        () async {
          // Arrange
          const combinations = [
            [1, 10],
          ...
          for (final combo in combinations) {
            when(
              mockBackendApiService.deleteWaypoint(combo[0], combo[1]),
            ).thenAnswer((_) async => {});
          }
    ...
    group('updateWaypointOrder', () {
      ...
      test('should update waypoint order successfully', () async {
        // Arrange
        when(
          mockBackendApiService.updateWaypointOrder(
            testHikeId,
            testWaypointId,
            newOrderIndex,
          ),
        ...
        // Act & Assert - should not throw
        await waypointRepository.updateWaypointOrder(
          testHikeId,
          testWaypointId,
          newOrderIndex,
        );
      ...
      test(
        ...
        () async {
          ...
          try {
            ...
            expect(
              e.toString(),
              contains('Fehler beim Aktualisieren der Wegpunkt-Reihenfolge'),
            );
            expect(e.toString(), contains(originalError));
          }
    ...
    group('Error Handling and Logging', () {
      test('should wrap all exceptions with German error messages', () async {
        ...
        when(
          mockBackendApiService.getWaypointsForHike(hikeId),
        ).thenThrow(Exception('Test error'));
        when(
          mockBackendApiService.addWaypoint(waypoint, hikeId),
        ).thenThrow(Exception('Test error'));
        when(
          mockBackendApiService.updateWaypoint(waypoint),
        ).thenThrow(Exception('Test error'));
        when(
          mockBackendApiService.deleteWaypoint(waypointId, hikeId),
        ).thenThrow(Exception('Test error'));
        when(
          mockBackendApiService.updateWaypointOrder(hikeId, waypointId, 1),
        ).thenThrow(Exception('Test error'));
      ...
      test('should preserve original error information', () async {
        ...
        try {
          ...
          expect(
            e.toString(),
            contains('Fehler beim Abrufen der Wegpunkte für Wanderung $hikeId'),
          );
        }
      });
    });
    ...
    group('Concurrent Operations', () {
      test('should handle multiple concurrent read operations', () async {
        ...
        for (int i = 0; i < hikeIds.length; i++) {
          when(
            mockBackendApiService.getWaypointsForHike(hikeIds[i]),
          ).thenAnswer((_) async => waypointLists[i]);
        }

        // Act
        final futures = hikeIds.map(
          (id) => waypointRepository.getWaypointsForHike(id),
        );
      ...
      test('should handle mixed concurrent operations', () async {
        ...
        when(
          mockBackendApiService.updateWaypointOrder(hikeId, waypoint.id, 2),
        ).thenAnswer((_) async => {});
        ...
        final futures = [
          waypointRepository.getWaypointsForHike(hikeId),
          waypointRepository
              .addWaypoint(waypoint, hikeId)
              .then((_) => <Waypoint>[]),
          waypointRepository.updateWaypoint(waypoint).then((_) => <Waypoint>[]),
          waypointRepository
              .updateWaypointOrder(hikeId, waypoint.id, 2)
              .then((_) => <Waypoint>[]),
        ];
    ...
    group('Edge Cases and Validation', () {
      test('should handle waypoints with extreme coordinate values', () async {
        ...
        when(
          mockBackendApiService.getWaypointsForHike(1),
      ...
      test(
        'should handle waypoints with very long names and descriptions',
        () async {
          // Arrange
          ...
          when(
            mockBackendApiService.addWaypoint(longNameWaypoint, 1),
          ).thenAnswer((_) async => {});
      ...
      test(
        'should handle waypoints with empty names and descriptions',
        () async {
          // Arrange
          ...
          when(
            mockBackendApiService.updateWaypoint(emptyWaypoint),
          ).thenAnswer((_) async => {});
      ...
      test('should handle waypoints with many images', () async {
        ...
        when(
          mockBackendApiService.addWaypoint(imageHeavyWaypoint, 1),
        ).thenAnswer((_) async => {});
      ...
      test('should handle zero and negative IDs', () async {
        ...
        for (final testCase in testCases) {
          when(
            mockBackendApiService.deleteWaypoint(testCase[0], testCase[1]),
          ).thenAnswer((_) async => {});
        }
      ...
      test('should handle null safety for optional parameters', () async {
        ...
        when(
          mockBackendApiService.addWaypoint(waypoint, 1, orderIndex: null),
        ).thenAnswer((_) async => {});
    ...
    group('Performance and Load Testing', () {
      test('should handle processing large number of waypoints', () async {
        // Arrange
        const hikeId = 1;
        final manyWaypoints = List.generate(
          1000,
          (index) => Waypoint(
            id: index,
            hikeId: hikeId,
