  ...
  group('ProfileRepository Tests', () {
    ...
    group('getUserProfileById', () {
      ...
      test('should use expired cache as fallback when backend fails', () async {
        ...
        when(mockLocalCacheService.getCachedProfileData(testUserId))
            .thenAnswer((_) async {
              callCount++;
              if (callCount == 1) return null; // First call returns null (cache miss)
        ...
        verify(mockBackendApiService.getUserProfileById(testUserId)).called(1);
        verify(mockLocalCacheService.getCachedProfileData(testUserId)).called(2); // Called twice for fallback
      });

      test('should throw exception when backend fails and no cache available', () async {
        // Arrange
    ...
    group('updateUserProfile', () {
      ...
      test('should cache updated profile after successful update', () async {
        ...
        when(mockBackendApiService.updateUserProfile(profile))
            .thenAnswer((_) async => {});
        when(mockLocalCacheService.cacheProfileData(profile, profile.id))
            .thenAnswer((_) async => {});
        ...
        // Assert
        verify(mockBackendApiService.updateUserProfile(profile)).called(1);
        verify(mockLocalCacheService.cacheProfileData(profile, profile.id)).called(1);
      });
    ...
    group('uploadProfileImage', () {
      ...
      test('should cache image locally after successful upload', () async {
        ...
        expect(result, expectedUrl);
        verify(mockBackendApiService.uploadProfileImage(testUserId, imageBytes, testFileExt)).called(1);
        verify(mockLocalCacheService.cacheProfileImage(testUserId, imageBytes, testFileExt)).called(1);
      });
    ...
    group('getProfileImageUrl', () {
      ...
      test('should return cached image path with file:// prefix when cache hit', () async {
        // Arrange
      ...
      test('should return backend URL and attempt caching when cache miss', () async {
        // Arrange
    ...
    group('Cache Management', () {
      test('clearUserCache should delegate to cache service', () async {
        // Arrange
        const userId = 'user123';
        when(mockLocalCacheService.clearUserCache(userId))
            .thenAnswer((_) async => {});
      ...
      test('clearAllCache should delegate to cache service', () async {
        // Arrange
        when(mockLocalCacheService.clearAllCache())
            .thenAnswer((_) async => {});
      ...
      test('getCacheStats should return cache statistics', () async {
        ...
        when(mockLocalCacheService.getCacheStats())
      ...
      test('hasNetworkConnection should delegate to cache service', () async {
        // Arrange
        when(mockLocalCacheService.hasNetworkConnection())
    ...
    group('Cache Integration Scenarios', () {
      test('should use cache-first strategy for profile data', () async {
        ...
        final cachedProfile = Profile(id: userId, firstName: 'Cached', lastName: 'Data');
        final backendProfile = Profile(id: userId, firstName: 'Backend', lastName: 'Data');
        ...
        when(mockLocalCacheService.cacheProfileData(backendProfile, userId))
            .thenAnswer((_) async => {});
        ...
        // Assert
        verify(mockBackendApiService.getUserProfileById(userId)).called(1); // Only once
        verify(mockLocalCacheService.getCachedProfileData(userId)).called(2); // Twice
        verify(mockLocalCacheService.cacheProfileData(backendProfile, userId)).called(1);
      });
      ...
      test('should use cache-first strategy for profile images', () async {
        ...
        when(mockLocalCacheService.getCachedProfileImagePath(userId))
            .thenAnswer((_) async {
              imageCallCount++;
        ...
        expect(result2, 'file://$cachedImagePath'); // From cache with file:// prefix

        // Assert
        verify(mockBackendApiService.getProfileImageUrl(userId)).called(1); // Only once
        verify(mockLocalCacheService.getCachedProfileImagePath(userId)).called(2); // Twice
      });
      ...
      test('should handle partial cache failures in image operations', () async {
        // Arrange
        ...
        final result = await profileRepository.uploadProfileImage(userId, imageBytes, 'jpg');

        // Assert
        expect(result, uploadedUrl);
        verify(mockBackendApiService.uploadProfileImage(userId, imageBytes, 'jpg')).called(1);
        verify(mockLocalCacheService.cacheProfileImage(userId, imageBytes, 'jpg')).called(1);
      });
    });
  });
}