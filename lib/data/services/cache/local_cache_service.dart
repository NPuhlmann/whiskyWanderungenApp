...
class LocalCacheService {
  ...
  static const String _profileImageTimestampKey =
      'cached_profile_image_timestamp';
  ...
  Future<String?> getCachedProfileImagePath(String userId) async {
    try {
      ...
      log(
        "✅ Gecachtes Profilbild gefunden für Benutzer: $userId, Pfad: $imagePath",
      );
      return imagePath;
    } catch (e) {
  ...
  Future<void> _manageCacheSize() async {
    try {
      ...
      if (totalSize > maxCacheSizeBytes) {
        log(
          "📊 Cache-Größe (${(totalSize / 1024 / 1024).toStringAsFixed(1)}MB) überschreitet Limit",
        );
        ...
        log(
          "✅ Cache-Größe nach Bereinigung: ${(totalSize / 1024 / 1024).toStringAsFixed(1)}MB",
        );
      }
    } catch (e) {
  ...
  Future<Map<String, dynamic>> getCacheStats() async {
    try {
      ...
      await _initializePrefs();
      final keys = _prefs!.getKeys();
      final profileDataCacheCount = keys
          .where((key) => key.contains('cached_profile_data'))
          .length;
