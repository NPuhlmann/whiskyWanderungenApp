...
class OfflineService {
  ...
  Future<void> cacheDataList<T>({
    ...
    try {
      ...
      log(
        "💾 Datenliste gecacht - Type: $type, Key: $listKey, Count: ${data.length}",
      );
    } catch (e) {
  ...
  Future<List<T>?> getCachedDataList<T>({
    ...
    try {
      ...
      if (isExpired) {
        log(
          "⏰ Gecachte Datenliste ist abgelaufen - Type: $type, Key: $listKey",
        );
        await _removeCachedItem(key);
        return null;
      }
      ...
      log(
        "✅ Gecachte Datenliste geladen - Type: $type, Key: $listKey, Count: ${data.length}",
      );
      return data;
    } catch (e) {
      log(
        "❌ Fehler beim Laden gecachter Datenliste ($type:$listKey): $e",
        error: e,
      );
      return null;
    }
  ...
  Future<void> clearCache({String? type}) async {
    try {
      ...
      log(
        "🧹 Cache gelöscht - Type: ${type ?? 'all'}, Keys: ${keysToRemove.length}",
      );
    } catch (e) {
  ...
  Future<Map<String, dynamic>> getCacheStats() async {
    try {
      ...
      for (final key in cacheKeys) {
        if (key.endsWith(_timestampSuffix) || key.endsWith(_metadataSuffix)) {
          continue;
        }
  ...
  Future<void> _manageCacheSizeForType(String type) async {
    try {
      ...
      log(
        "🗑️ Cache-Größe verwaltet - Type: $type, Entfernt: $itemsToRemove Items",
      );
    } catch (e) {
  ...
  Future<void> performMaintenanceCleanup() async {
    try {
      ...
      log(
        "✅ Maintenance cleanup completed - Removed $removedCount expired items",
      );
    } catch (e) {
