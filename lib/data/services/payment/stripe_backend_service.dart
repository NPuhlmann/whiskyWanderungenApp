...
class StripeBackendService {
  ...
  static StripeBackendService get instance =>
      _instance ??= StripeBackendService._internal();
  ...
  Future<Map<String, dynamic>> createPaymentIntent({
    ...
    try {
      ...
      if (secretKey == null || !secretKey.startsWith('sk_test_')) {
        throw ArgumentError(
          'Valid Stripe secret key is required for backend operations',
        );
      }
      ...
      final body = {
        ...
        'currency': currency,
        'confirm':
            'false', // Don't auto-confirm, let client handle confirmation
      ...
      dev.log(
        '🔄 Creating real Stripe payment intent for €${amount.toStringAsFixed(2)}...',
      );
...
class StripeBackendException implements Exception {
  ...
  @override
  String toString() =>
      'StripeBackendException: $message${code != null ? ' (code: $code)' : ''}';
}
